...
\begin{abstract}
We identify parasitic liquidity, a class of emission extraction from concentrated liquidity decentralised exchanges (CL DEXs) that distribute protocol-native tokens to staked positions. It exploits a measurement gap: gauges reward instantaneous staked liquidity at a single tick, whereas swap facilitation requires sustained tradeable depth. The mechanism operates at the emission layer and is distinct from sandwich attacks, front-running, just-in-time liquidity, and arbitrage.

We derive the vulnerability from the public source of two architecturally distinct emission systems --- Slipstream CL gauges and PancakeSwap's \texttt{PancakeV3LmPool}. Both inherit the Synthetix reward-accumulator pattern, originally designed for fungible staking, and apply the same recurrence to non-fungible concentrated liquidity positions; neither imposes a warmup, minimum width, or utilisation condition on the accrual path. A Foundry proof-of-concept suite, executed against the unmodified contracts on Base mainnet, confirms that rewards accrue from the first block and that reward rate per unit of liquidity is independent of both stake duration and tick width.

Aggregate on-chain analysis indicates that single-tick-width positions rebalancing every block account for a material share of staked liquidity in major pools, reaching 63\% of in-range staked liquidity in one observed WETH/USDC pool. We formalise the profitability conditions, quantify the dilution imposed on legitimate liquidity providers, and propose two gauge-level remediations, each evaluated against fork-measured baselines. Any CL DEX whose gauge follows this accumulator pattern without duration, width, or utilisation checks appears structurally exposed to the same extraction mechanism.
\end{abstract}
...
The commonly documented MEV extraction classes listed below operate at the swap execution layer:
...
\item Foundry proof-of-concept suites against two independent protocol implementations on Base mainnet, with aggregate on-chain evidence consistent with use at scale (\S\ref{sec:poc}).
...
PancakeSwap's \texttt{MasterChefV3} arrived at the same vulnerability via parallel evolution, applying the Synthetix accumulator (used since MasterChef~V1) to their Uni~V3 fork. Any protocol that distributes emissions under this accumulator pattern on a concentrated liquidity pool appears structurally exposed to the same extraction mechanism.
...
A single-tick position minted at the current tick covers one tick spacing for one block. Even if swap volume passes through that tick, the fee capture from a two-second window is small relative to the emission income earned in the same period. The Foundry tests in \S\ref{sec:poc} measure the emission side of this comparison; the fee side follows from observed pool volume at the relevant fee tier.
...
On-chain analysis of major CL pools across affected protocols, conducted between March and April 2026. Specific pool identifiers, address-level data, and exact observation windows are omitted from the aggregate figures below.

\begin{itemize}[leftmargin=2em]
\item In the largest WETH/USDC CL100 pool on one observed protocol, single-tick-width positions account for 63.4\% of in-range staked liquidity.
\item Across 50 sampled blocks on this pool, 46 (92\%) contained zero \texttt{Swap} events whilst single-tick positions remained staked; emissions accrued in those blocks.
\item On a second protocol, single-tick positions in the four highest-emission pools capture an estimated 18.1\% of that protocol's emissions on the chain in question.
\item Across the observed protocols and chains, positions matching this pattern are simultaneously present in more than 20 gauged pools.
\item Positions matching this pattern have been continuously present in the observed pools over the observation window.
\end{itemize}
...
Applied to fork-measured pool state, the capital-efficiency penalty scales faster than linearly with enforced width. At \texttt{minWidth} = $2\times$ tick spacing, a parasitic operator needs $2\times$ more capital for the same liquidity. At $5\times$ tick spacing, the requirement rises to $6\times$. At $10\times$, $15\times$.
...
Applying the two fixes analytically to fork-measured reward accrual yields the following effective-yield ratios:
...
Any CL DEX whose gauge uses this accumulator pattern without duration, width, or utilisation checks appears structurally exposed to the same extraction mechanism. The proof-of-concept suites in \S\ref{sec:foundry} verify the underlying properties against two architecturally distinct implementations on Base. The capital requirement is a single position plus gas reserves, deployable from any address.
...
Parasitic liquidity is a structural vulnerability in the emission distribution layer of CL DEXs whose gauges use the Synthetix-style instantaneous-stake accumulator. Gauges measure instantaneous liquidity. Protocols intend to incentivise sustained swap depth. The gap between these two quantities is exploitable.

The vulnerability is demonstrated against unmodified mainnet contracts. Aggregate on-chain evidence shows the pattern in continuous use across the observed pools. The strategy is linearly scalable with a low economic floor.

The remediations analysed here are minimum position widths and reward warmup periods; more comprehensive gauge designs are possible and are out of scope. Absent gauge-level modifications, further adoption of the pattern is plausible under the present incentive structure.
